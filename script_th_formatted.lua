...
local Templates = {
    ...
    --// Library \\--
    Window = {
        Title = "{🖕🏼}ขี่สัตว์เลี้ยง | USB HuB",
        Footer = "ไม่มีฟุตเตอร์",
        CopyableFooter = true,
...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        if ParentObj.Type == "Button" or ParentObj.Type == "SubButton" then
            assert(Info.Mode == "กด", "KeyPicker on Buttons can only be applied with the 'กด' mode.")
        ...
        local KeybindsToggle = { Normal = KeyPicker.Mode ~= "สลับ" }
        do
        ...
        function KeyPicker:Update()
            ...
            if KeyPicker.Mode == "สลับ" and ParentObj.Type == "Toggle" and ParentObj.Disabled then
                KeybindsToggle:SetVisibility(false)
                return
            end

            local State = KeyPicker:GetState()
            local ShowToggle = Library.ShowToggleFrameInKeybinds and KeyPicker.Mode == "สลับ"
        ...
        function KeyPicker:GetState()
            if KeyPicker.Mode == "เสมอ" then
                return true
            elseif KeyPicker.Mode == "กดค้าง" then
        ...
        table.insert(KeyPicker.Connections, UserInputService.InputBegan:Connect(function(Input: InputObject)
            ...
            if KeyPicker.Mode == "สลับ" then
                if HoldingKey then
                    KeyPicker.Toggled = not KeyPicker.Toggled
                    KeyPicker:DoClick()
                end
            elseif KeyPicker.Mode == "กด" then
                if HoldingKey then
                    KeyPicker:DoClick()
                end
            end
    ...
    function Funcs:AddColorPicker(Idx, Info)
        ...
        table.insert(ColorPicker.Connections, CopyColorButton.MouseButton1Click:Connect(function()
            ...
            CopyColorButton.Text = "คัดลอกสีแล้ว"
        ...
        table.insert(ColorPicker.Connections, PasteColorButton.MouseButton1Click:Connect(function()
            ...
            if not Library.CopiedColor then
                PasteColorButton.Text = "ไม่มีอะไรให้วาง"
            else
                ColorPicker:SetValueRGB(Library.CopiedColor[1], Library.CopiedColor[2])
                PasteColorButton.Text = "วางสีแล้ว"
            end
...
do
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        if Info.Expandable ~= false then
            ...
            Library:AddTooltip("ขยาย", nil, ExpandButton)
        end
...
function Library:RefreshNotificationHistory()
    ...
    for _, Entry in Library.NotificationHistory do
        ...
        Library:AddTooltip("คลิกเพื่อคัดลอก", nil, Card)
        ...
        Card.MouseButton1Click:Connect(function()
            ...
            CopiedLabel.Text = Ok and "คัดลอกแล้ว!" or "ไม่มีคลิปบอร์ด"
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        Library.KeybindFrame, Library.KeybindContainer = Library:AddDraggableMenu("ปุ่มลัด")
        ...
        if WindowInfo.Minimizable then
            ...
            Library:AddTooltip("ย่อ", nil, MinimizeButton)
            ...
            Library:AddTooltip("คืนค่า", nil, RestoreButton)
        ...
        if not WindowInfo.DisableNotificationBell then
            ...
            Library:AddTooltip("ประวัติการแจ้งเตือน", nil, BellButton)
        ...
        local function AddFooterSegment(Info)
            ...
            Library:AddTooltip("คัดลอกไปยังคลิปบอร์ด", nil, CopyButton)
    ...
    if Library.IsMobile then
        local ToggleButton = Library:AddDraggableButton("สลับ", function()
            Library:Toggle()
        end, true, true)

        local LockButton = Library:AddDraggableButton("ล็อก", function(self)
            Library.CantDragForced = not Library.CantDragForced
            self:SetText(Library.CantDragForced and "ปลดล็อก" or "ล็อก")
        end, true, true)
...
function Library:CreateLoading(LoadingInfo)
    ...
    --// ScreenGui \\--
    local ScreenGui = New("ScreenGui", {
        Name = "{🖕🏼}ขี่สัตว์เลี้ยง | USB HuB Loading",
        DisplayOrder = 999,
        ResetOnSpawn = false
    })
    ParentUI(ScreenGui)
...
function Library:CreateUnsupportedScreen(Info)
    ...
    local Tab = Window:AddTab({ Name = "ไม่รองรับ", Icon = "shield-alert" })
    ...
    local InfoBox = Tab:AddRightGroupbox("ข้อมูล", "info")
